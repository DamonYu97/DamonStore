...
<body>
            ...
            <div class="navbar-default sidebar col-lg-3" role="navigation">
                <div class="sidebar-nav container">
                    <ul class="nav nav-pills nav-stacked">
                        ...
                        <li class="active"><a href="${PATH}/member/allOrders"> All orders</a></li>
                        ...
                        <div class="modal fade" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="confirmModalLabel">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <h4>Make sure you have received our product before pressing 'confirm' button!</h4>
                                    </div>
    ...
    <script type="text/javascript">
            ...
            success:function (data) {
                ...
                for (let i =0; i < data.length; i++) {
                    ...
                    switch (data[i].order.status) {
                        case 1:
                            status = 'Unpaid';
                            operationButton = '<button class="btn btn-danger" data-toggle="modal" data-target="#payModal" data-price=\"' +
                                data[i].order.totalPrice + '\" data-id='+data[i].order.id+'>Pay</button>\n';
                            break;
                        case 2:
                            status = 'Unshipped';
                            operationButton =  '<button class="btn btn-danger">Remind damon</button>\n';
                            break;
                        case 3:
                            status = 'Unreceived';
                            operationButton = '<button class="btn btn-danger" data-toggle="modal" data-target="#confirmModal" data-price=\"' +
                                data[i].order.totalPrice + '\" data-point=' + data[i].order.totalPoint +
                                ' data-id='+data[i].order.id+'>Received</button>\n';
                            break;
                        case 4:
                            status = 'Done';
                            break;
        ...
        $('#confirmModal').on('show.bs.modal', function (event) {
            ...
            let goConfirmButton = '<button type="button" class="btn btn-primary" onclick="received(' + orderID + ',' +
                price + ',' + point + ')">Confirm</button>';
            $('#confirmFooter').html(goConfirmButton);
        });

        //确认收货
        function received(orderID,price,point) {
            $.ajax({
                type: 'POST',
                url: '${PATH}/member/order/received',
                data: {"orderID":orderID,"price":price,"point":point},
                dataType: 'json',
                success:function (data) {
                    console.log(data.result);
                    window.location.href = "${PATH}/member/allOrders";
